...
class OnlineRecognizer(
    ...
    companion object {
        init {
            // 先把共享的 ONNX Runtime 显式载入全局命名空间，再载 sherpa 的 jni。
            // 原因见 App.kt：libsherpa-onnx-jni.so 与 libonnxruntime4j_jni.so 都
            // DT_NEEDED 同一个 "libonnxruntime.so"，而 loadLibrary 默认 RTLD_LOCAL，
            // 先加载的一方会把该符号据为己有，后加载方报
            // "cannot locate symbol OrtGetApiBase"。先全局载入即可让双方都解析成功。
